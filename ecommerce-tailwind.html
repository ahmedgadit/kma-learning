...
<body class="bg-white-50 flex flex-col text-blue-900 h-svh">
    ...
    <div id='container' class="flex grow flex-column">
        <div class="w-72 box-border p-8 bg-red-100">
            <h1 class="text-blue-400 text-2xl">Sidebar</h1>
        </div>
        <div id='mainBox' class="flex w-full gap-4">
            <div class='bg-white-500 text-black-500 p-4 rounded-md shadow-2xl slideDown' style='width: 30%;'>
                <form id='ecommerceForm' onsubmit='event.preventDefault();'
                    ...
                    <button type='button' onclick="sumbitCheckout()" id='submitBtn' class='bg-blue-600 text-white-200 p-2 border-0 hover:bg-blue-700 active:bg-blue-700 rounded-md'>Checkout</button>
                </form>
                <div>
...
<script>
    ...
    function renderCarts() {
        ...
        checkoutForm.cart_details.forEach((x) => {
            var priceBox = x.discounted_price < x.price ? `<div class='text-seaGreen-500'>
                        <span class='text-red-600 line-through'> `+ x.price + `</span>
                        `+ x.discounted_price + `
                    </div>` : `<div class='text-seaGreen-500'>
                        `+ x.price + `
                    </div>` ;

            cartHtml += `<div class="box bg-red-50 font-medium text-blue-900">
                    <div style="padding-left:10px; border-radius: 50%; padding-right: 10px; display:flex; justify-content: space-between; align-items: center;">
                        ...
                        <div>
                            <button class="text-red-500" onclick="removeItemCart(`+ x.id + `)">
                                X
                            </button>
                        </div>
                    </div>
        ...
        products.forEach((x) => {
            ...
            productHtml += `<div class='bg-white-500 text-black-500 p-4 rounded-md shadow-2xl slideDown'>
                    ...
                    <div class=''>
                        ...
                        <button class='bg-blue-400 text-white-200 font-medium rounded-lg w-32 py-1' onclick='addToCart(`+ x.id + `)'>Add to Cart</button>
